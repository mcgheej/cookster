...
@Component({
  ...
  template: `
    @if (selectedActivity(); as activity) {
      <div class="ml-1">
        <div
          class="inline-block size-2.5 rounded-[50%]"
      ...
      <div class="pl-4.5 text-sm select-none">{{ activity.name }}</div>
      <div class="pl-4.5 pt-2 font-bold text-sm select-none">Time:</div>
      <div class="pl-4.5 text-sm select-none">{{ timeString() }}</div>
      <div class="pl-4.5 pt-2 font-bold text-sm select-none">Resource:</div>
      <div class="pl-4.5 text-sm select-none">{{ resourceName() }}</div>
      <div class="pl-4.5 pt-2 font-bold text-sm select-none">Description:</div>
      <div class="ml-3.25 mt-1 border border-gray-300 w-68.25 h-20 overflow-auto">
        <div class="pl-1 pr-1 text-sm select-none">{{ activity.description || 'No description' }}</div>
      </div>
      <div class="pl-4.5 pt-2 font-bold text-sm select-none">Actions:</div>
      <div class="border border-gray-300 mt-1 ml-4 w-68.25 h-37.5 overflow-auto">
      ...
      <div class="pl-4.5 pt-2 font-bold text-sm select-none">Start/End Messages:</div>
      <div class="pl-4.5 text-sm select-none">{{ getStartMessage(activity) }}</div>
      <div class="pl-4.5 text-sm select-none">{{ getEndMessage(activity) }}</div>
