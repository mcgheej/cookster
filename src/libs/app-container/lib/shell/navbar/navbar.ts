...
@Component({
  ...
  template: `
    <nav class="flex flex-wrap items-center w-full py-0 px-4 text-xl font-medium text-white bg-[#303F9F]">
      ...
      <ul class="flex text-base text-white justify-between pt-0">
        ...
        @if (loggedIn()) {
          <li>
            <div
              class="px-4 py-2 block hover:text-yellow-500 cursor-pointer select-none box-border"
              (click)="logout.emit()">
              Logout
            </div>
          </li>
  ...
  styles: [
    `
      .active-link {
        @apply underline decoration-2 underline-offset-8 decoration-[#FFFFFF];
      }
    `,
  ],
